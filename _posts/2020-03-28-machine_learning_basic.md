...
- 알고리즘 f를 만들어내는 방식의 차이다.
	- 명시적 프로그래밍
		...
		- 명시적인 로직이 곧 알고리즘이 된다.
			- *몸무게 = 키 - 110*
	- 머신러닝
		- 명시적 로직은 아닌, 사람이 어느정도 추상화시킨 로직들을 세우고, 컴퓨터는 로직들이 최적의 속성 값을 가지도록 입/출력 데이터를 통해 학습한다.
		- 적절한 속성 값을 가진 로직들이 알고리즘이 된다.
			- *몸무게 = a x 키 + b*
			- *로직에 키와 몸무게 데이터들을 넣어 속성 값 a, b를 찾아낸다.*
	- 딥러닝
		- 사람이 로직을 세우지 않고, 일반화된 신경망이 로직을 대체한다. 컴퓨터는 신경망이 최적의 파라미터 값을 가지도록 입/출력 데이터를 통해 학습한다.
...
## 정확도(Accuracy)
- 얼마나 정확히 예측했나?
- 예측한 정답 중에 실제 정답 수 + 예측한 오답 중에 실제 오답 수
- *100개의 데이터 중에 50개가 정답인데, 25개를 정답으로 예측했고, 10개만 실제 정답이었다면,*
	- *정답을 정답으로 예측 = 10개, 오답을 정답으로 예측 = 15개, 정답을 오답으로 예측 = 40개, 오답을 오답으로 예측 = 35개*
	- *정확도 = 45(10+35)/100 = 0.45*

## 정밀도(Precision)
- 얼마나 정밀하게 정답을 예측했나? 예측한 정답 중에 실제 정답의 비율은 얼마나 되는가?
- 모델이 정답이라고 예측한 데이터 중에 실제 정답 데이터의 비율
- 100개의 데이터 중에 50개가 정답인데, 모델이 80개를 정답이라고 예측했고, 그 중 40개가 실제 정답이었다면,
	- *정밀도 = 40/80 = 0.5*
	- *재현률 = 40/50 = 0.8*
...
- 100개의 데이터 중에 50개가 정답인데, 모델이 2개를 정답이라고 예측했고, 2개 모두 실제 정답이었다면,  
	- *정밀도 = 2/2 = 1.0*
	- *재현률 = 2/50 = 0.04*

## F1-점수(F1-Score)
- 정확도와 정밀도의 조화평균(역수의 산술평균 값의 역수)
- 편향을 고려해 평균을 표현할 수 있다.
- 정밀도가 1.0, 재현률이 0.04라고 한다면,
	- *F1-Score = 2 x 0.04 / 1.04 = 0.07692307*
